...
class DartsTest(unittest.TestCase):
    ...
    def test_does_not_accept_non_real_numbers(self):
        input_data = [(1j, 0.5 + 1j), ("Hello", "Bye"), ("how", 1j), (True, "hi")]

        for variant, (x, y) in enumerate(input_data, start=1):
            with self.subTest(f"variation #{variant}", x=x, y=y):
