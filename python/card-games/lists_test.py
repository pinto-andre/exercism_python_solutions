...
class CardGamesTest(unittest.TestCase):
    ...
    def test_only_allows_positive_integers_get_rounds(self):
        input_data = [-2, 1.3, -7.5, -10, -5]

        for variant, number in enumerate(input_data, start=1):
            with self.subTest(f"variation #{variant}", number=number):
                with pytest.raises(ValueError):
                    get_rounds(number)

    @pytest.mark.task(taskno=9)
    def test_only_accepts_lists_with_more_than_one_element(self):
        input_data = [[1], [1.3], [], [-10], [-5]]

        for variant, input_hand in enumerate(input_data, start=1):
            with self.subTest(f"variation #{variant}", input_hand=input_hand):
                with pytest.raises(ValueError):
                    average_even_is_average_odd(input_hand)
