...
class PangramTest(unittest.TestCase):
    ...
    def test_only_allows_strings(self):
        input_data = [-1, 1.5, True]

        for variant, sentence in enumerate(input_data, start=1):
            with self.subTest(f"variation #{variant}", sentence=sentence):
